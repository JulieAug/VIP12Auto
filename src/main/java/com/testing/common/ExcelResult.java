...
public class ExcelResult {
	...
	//成员变量列号，用于进行结果统计时，哪一列是结果列。
	private int resultCol=10;

	//设置结果列号
	public void setResultCol(int column){
		resultCol=column;
	}
	...
	public List<Map<String, String>> Sumarry(String path, String startTime) {
		try {
			...
			String endtime = AutoTools.timeStampString("yyyy-MM-dd HH:mm:ss");
...
//								System.out.println(line + "分组信息");
						...
						// 第一列信息和第二列信息均为空的行，即用例行，这时开始进行用例数、通过数、状态的统计。
							...
							if (line.get(resultCol) == null || line.get(resultCol).trim().length() ==0) {
								flag = false;
							}
							// 执行结果不为空，则将用例统计数自增
							else {
...
//							System.out.println(line);
								// 如果通过，则通过数和总通过数均自增
								if (line.get(resultCol).toLowerCase().equals("pass")) {
									totalpass++;
									detailpass++;
								} else {
