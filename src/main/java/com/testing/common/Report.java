...
public class Report {
	...
	/***
	 * 用于完成结果文件的数据统计并发送邮件报告。
	 * @param resultFileName
	 * @param startTime
	 */
	public static void sendreport (String resultFileName,String startTime)  {
		...
		List<Map<String,String>> resultList = excel.Sumarry(resultFileName+".xlsx",startTime);
		...
		//发送邮件
		try {
			mail.send(content,resultFileName+".xlsx");
		} catch (Exception e) {
	...
	//通过模板，生成HTML报告内容
	public static String makeReport(List<Map<String,String>> resultlist) {
		//邮件开头
		...
		//替换图片和发件人,
		content=content.replace("http://47.105.110.138/imgs/logo_big.png", "https://rescdn.qqmail.com/zh_CN/htmledition/images/spacer1e9c5d.gif");
		content = content.replace("Roy", "Julie");
